...
// ===================== DIRECTOR USER FROM localStorage =====================
const dirUser = JSON.parse(localStorage.getItem('user') || '{}');
if (!dirUser || !dirUser.id) { window.location.href = 'SignIn.html'; }

// Fill director name everywhere
document.querySelectorAll('.banner-name').forEach(el => el.textContent = dirUser.name || 'Director');
document.querySelectorAll('.settings-user-name').forEach(el => el.textContent = dirUser.name || 'Director');
const initials = (dirUser.name || 'D').split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
document.querySelectorAll('.settings-user-avatar').forEach(el => el.textContent = initials);

// ===================== STATE =====================
let approvalData     = [];   // PENDING_DIRECTOR leaves only
let allLeavesData    = [];   // All leaves for history
...
async function loadAllData() {
  try {
    ...
    // Director sees ONLY leaves with status PENDING_DIRECTOR ← FIXED
    approvalData  = all.filter(l => l.status === 'PENDING_DIRECTOR').map(mapLeave);

    // History = all leaves that went beyond HOD (days > 10)
    allLeavesData = all.filter(l => l.days > 10).map(mapLeave);

    updateBannerCounts(all);
    renderDeptOverview(all);
    renderApprovalTable();
    renderHistSnapshot();
    renderHistoryTable();
    updateNotifBadge();
...
// ===================== BANNER COUNTS =====================
function updateBannerCounts(all) {
  const pending  = all.filter(l => l.status === 'PENDING_DIRECTOR').length;
  const approved = all.filter(l => l.status === 'APPROVED_BY_DIRECTOR').length;
  const total    = all.filter(l => l.days > 10).length;
...
// ===================== DEPT OVERVIEW STATS =====================
function renderDeptOverview(all) {
  ...
  const stats = [
    ...
    { icon:'⏳', label:'Pending (Director)',      val: pending,  bg:'#fef3c7', color:'#d97706' },
    { icon:'✅', label:'Approved by Director',   val: approved, bg:'#d1fae5', color:'#059669' },
...
function renderApprovalTable() {
  ...
  // ← FIXED: filter by status properly
  const filtered = activeFilter === 'all'
    ? approvalData
    : activeFilter === 'Pending'
      ? approvalData.filter(r => r.rawStatus === 'PENDING_DIRECTOR')
      : activeFilter === 'Approved'
        ? approvalData.filter(r => r.rawStatus === 'APPROVED_BY_DIRECTOR')
        : approvalData.filter(r => r.rawStatus === 'REJECTED' || r.rawStatus === 'CANCELLED');

  const pending = approvalData.filter(r => r.rawStatus === 'PENDING_DIRECTOR').length;
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:32px;color:var(--muted)">🎉 No pending requests for Director</td></tr>`;
    return;
  }

  filtered.forEach(row => {
    const tr = document.createElement('tr');
    const statusClass = row.rawStatus === 'PENDING_DIRECTOR'   ? 'badge-pending'
                      : row.rawStatus === 'APPROVED_BY_DIRECTOR' ? 'badge-approved'
                      : row.rawStatus === 'REJECTED'             ? 'badge-rejected'
                      : 'badge-pending';

    const showActions = row.rawStatus === 'PENDING_DIRECTOR';
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn-view-app" onclick="viewApplication(${row.id})">👁 View</button>
      </td>
      <td>
        ${showActions ? `
...
function viewApplication(id) {
  ...
  const statusClass = row.rawStatus === 'PENDING_DIRECTOR' ? 'badge-pending'
    : row.rawStatus === 'APPROVED_BY_DIRECTOR' ? 'badge-approved' : 'badge-rejected';
...
// ===================== HISTORY SNAPSHOT =====================
function renderHistSnapshot() {
  const container = document.getElementById('histSnapshot') || document.getElementById('classSnapshot');
  if (!container) return;
  ...
  const approved = data.filter(r => r.rawStatus === 'APPROVED_BY_DIRECTOR').length;
  const pending  = data.filter(r => r.rawStatus === 'PENDING_DIRECTOR').length;
...
function updateNotifBadge() {
  ...
  if (badge) badge.textContent = approvalData.filter(r => r.rawStatus === 'PENDING_DIRECTOR').length;
}
...
async function changePassword() {
  ...
  try {
    const res = await fetch(`${API}/users/${dirUser.id}/password`, {
      method: 'PATCH',
...
// ===================== PROFILE FILL =====================
document.getElementById('profileName')  && (document.getElementById('profileName').value  = dirUser.name  || '');
document.getElementById('profileEmail') && (document.getElementById('profileEmail').value = dirUser.email || '');
